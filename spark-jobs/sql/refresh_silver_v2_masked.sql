...
-- ВЕРСИЯ С МАСКИРОВАННЫМИ ДАННЫМИ (signature.tin / signature.full_name)
...
-- МАСКИРОВАННЫЕ ДАННЫЕ: signature.tin и signature.full_name (777777...)
...
SELECT
    ...
    COALESCE(
        NULLIF(get_json_object(data, '$.company_tin'), ''),
        NULLIF(get_json_object(signature, '$.tin'), '')
    ) as company_tin,
    COALESCE(
        NULLIF(get_json_object(data, '$.company_name'), ''),
        NULLIF(get_json_object(signature, '$.full_name'), '')
    ) as company_name,
...
-- Проверка маскированных данных (должны быть 777777...)
SELECT company_tin, company_name, COUNT(*) as cnt
FROM iceberg.silver.service_report_v2
WHERE year = 2026
GROUP BY company_tin, company_name
ORDER BY cnt DESC
LIMIT 10;
